...
import { createPostVersionNext, getLatestPostVersion } from "@/lib/models/postVersion";
...
export async function POST(
  ...
  const promptNote = body.promptNote ?? "";
  ...
  const previousVersion = getLatestPostVersion(uid, postId);

  // Build prompt note with previous version context
  let fullPromptNote = promptNote;
  if (previousVersion) {
    fullPromptNote = `이전 버전 제목: ${previousVersion.title}\n이전 버전 일부: ${previousVersion.contentMarkdown.slice(0, 500)}\n\n${promptNote}`;
  }
